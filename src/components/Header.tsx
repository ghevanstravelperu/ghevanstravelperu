...
import { HeaderSocialLinks } from "./SocialIcons";
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 overflow-visible border-b border-stone-200/80 bg-cream/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-5 sm:px-6 sm:py-6 lg:gap-4">
        ...
        <nav
          className="hidden shrink-0 items-center gap-x-1 rounded-full border border-stone-200/70 bg-white/55 px-2 py-1.5 shadow-sm backdrop-blur-sm xl:flex"
          aria-label="Primary"
        >
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`whitespace-nowrap rounded-full px-3.5 py-1.5 text-[0.8rem] font-semibold uppercase tracking-[0.12em] transition ${
                  active
                    ? "bg-navy text-white"
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          <a
            ...
            className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-[#25D366] px-3.5 py-2 text-sm font-semibold whitespace-nowrap text-white shadow-[0_8px_20px_-6px_rgba(37,211,102,0.7)] transition duration-300 hover:-translate-y-0.5 hover:bg-[#20bd5a] hover:shadow-[0_14px_28px_-8px_rgba(37,211,102,0.85)] active:translate-y-0"
          >
            ...
            <span className="relative">{tWhatsapp("headerCta")}</span>
          </a>
          <HeaderSocialLinks />
        </div>
      </div>
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`shrink-0 whitespace-nowrap rounded-full px-3.5 py-1.5 text-[0.7rem] font-semibold uppercase tracking-[0.12em] transition ${
                  active
                    ? "bg-navy text-white"
